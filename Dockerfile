...
RUN apt update && apt install sudo freerdp2-x11 yad fvwm xterm xinit mingetty polkitd wpasupplicant nano udiskie mc mtr firmware-linux firmware-linux-nonfree firmware-iwlwifi firmware-realtek firmware-atheros firmware-brcm80211 firmware-b43-installer ffmpeg pipewire-audio -y
